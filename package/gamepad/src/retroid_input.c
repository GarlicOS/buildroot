...
static int32_t get_retroid_input_axis_min_value(int axis)
{
	switch (axis)
	{
		case RETROID_INPUT_AXIS_LEFT_ANALOG_HORIZONTAL:
		case RETROID_INPUT_AXIS_LEFT_ANALOG_VERTICAL:
		case RETROID_INPUT_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case RETROID_INPUT_AXIS_RIGHT_ANALOG_VERTICAL:
			return RETROID_INPUT_ANALOG_VALUE_MIN;
		default:
			break;
	}
...
static int32_t get_retroid_input_axis_max_value(int axis)
{
	switch (axis)
	{
		case RETROID_INPUT_AXIS_LEFT_ANALOG_HORIZONTAL:
		...
		case RETROID_INPUT_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case RETROID_INPUT_AXIS_RIGHT_ANALOG_VERTICAL:
			return RETROID_INPUT_ANALOG_VALUE_MAX;
		default:
			break;
	}
...
static int translate_scancode(int scancode)
{
	switch (scancode)
	{
		case RETROID_INPUT_SCANCODE_HOME:
			return MERGED_SCANCODE_HOME;
		case RETROID_INPUT_SCANCODE_A:
			return MERGED_SCANCODE_B;
		case RETROID_INPUT_SCANCODE_B:
			return MERGED_SCANCODE_A;
		case RETROID_INPUT_SCANCODE_Y:
			return MERGED_SCANCODE_X;
		case RETROID_INPUT_SCANCODE_X:
			return MERGED_SCANCODE_Y;
		case RETROID_INPUT_SCANCODE_L1:
			return MERGED_SCANCODE_L1;
		case RETROID_INPUT_SCANCODE_R1:
			return MERGED_SCANCODE_R1;
		case RETROID_INPUT_SCANCODE_SELECT:
			return MERGED_SCANCODE_SELECT;
		case RETROID_INPUT_SCANCODE_START:
			return MERGED_SCANCODE_START;
		case RETROID_INPUT_SCANCODE_L3:
			return MERGED_SCANCODE_L3;
		case RETROID_INPUT_SCANCODE_R3:
			return MERGED_SCANCODE_R3;
		case RETROID_INPUT_SCANCODE_VOLUME_MINUS:
			return MERGED_SCANCODE_VOLUME_MINUS;
		case RETROID_INPUT_SCANCODE_VOLUME_PLUS:
			return MERGED_SCANCODE_VOLUME_PLUS;
		case RETROID_INPUT_SCANCODE_POWER:
...
static void scale_and_translate_axis(struct input_event * ev)
{
	...
	// Scale range
	int32_t new_value = scale_axis_value(ev->value, get_retroid_input_axis_min_value(ev->code), get_retroid_input_axis_max_value(ev->code), get_merged_axis_min_value(new_code), get_merged_axis_max_value(new_code));
...
static int convert_to_axis_event(struct input_event * ev)
{
	switch (ev->code)
	{
		case RETROID_INPUT_SCANCODE_L2:
		case RETROID_INPUT_SCANCODE_R2:
			// Convert digital to analog triggers
			ev->type = EV_ABS;
			ev->code = ev->code == RETROID_INPUT_SCANCODE_L2 ? MERGED_AXIS_LEFT_TRIGGER : MERGED_AXIS_RIGHT_TRIGGER;
			ev->value = ev->value ? MERGED_TRIGGER_VALUE_MAX : MERGED_TRIGGER_VALUE_MIN;
			return 1;
		case RETROID_INPUT_SCANCODE_DPAD_UP:
			// Convert dpad to analog axis
			ev->type = EV_ABS;
			ev->code = MERGED_AXIS_DPAD_VERTICAL;
			ev->value = ev->value ? MERGED_DPAD_VALUE_MIN : 0;
			return 1;
		case RETROID_INPUT_SCANCODE_DPAD_DOWN:
			// Convert dpad to analog axis
			ev->type = EV_ABS;
			ev->code = MERGED_AXIS_DPAD_VERTICAL;
			ev->value = ev->value ? MERGED_DPAD_VALUE_MAX : 0;
			return 1;
		case RETROID_INPUT_SCANCODE_DPAD_LEFT:
			// Convert dpad to analog axis
			ev->type = EV_ABS;
			ev->code = MERGED_AXIS_DPAD_HORIZONTAL;
			ev->value = ev->value ? MERGED_DPAD_VALUE_MIN : 0;
			return 1;
		case RETROID_INPUT_SCANCODE_DPAD_RIGHT:
			// Convert dpad to analog axis
			ev->type = EV_ABS;
			ev->code = MERGED_AXIS_DPAD_HORIZONTAL;
			ev->value = ev->value ? MERGED_DPAD_VALUE_MAX : 0;
...
void *simulate_volumekey_repeat_retroid_input(void *volumekey_struct)
{
...
void merge_retroid_input_inputs(int merged_gamepad)
{
	...
	int gpio_keys = open(RETROID_INPUT_GPIO_KEYS, O_RDONLY | O_NONBLOCK);
	...
	int gamepad = open(RETROID_INPUT_GAMEPAD, 0x102);
			...
			if (FD_ISSET(gpio_keys, &wfds))
			{
				...
				if (read(gpio_keys, &ev, sizeof(ev)) == sizeof(ev))
				{
					...
					if (ev.code == KEY_VOLUMEUP)
					{
						...
						{
							pthread_create(&volumeup_thread, NULL, &simulate_volumekey_repeat_retroid_input, (void *)&volumeup_struct);
						}
					}
					if (ev.code == KEY_VOLUMEDOWN)
					{
						...
						{
							pthread_create(&volumedown_thread, NULL, &simulate_volumekey_repeat_retroid_input, (void *)&volumedown_struct);
						}
					}
