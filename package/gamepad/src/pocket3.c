...
static int32_t get_pocket3_axis_min_value(int axis)
{
	switch (axis)
	{
		case POCKET3_AXIS_LEFT_ANALOG_HORIZONTAL:
		case POCKET3_AXIS_LEFT_ANALOG_VERTICAL:
		case POCKET3_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case POCKET3_AXIS_RIGHT_ANALOG_VERTICAL:
			return POCKET3_ANALOG_VALUE_MIN;
		default:
			break;
	}
...
static int32_t get_pocket3_axis_max_value(int axis)
{
	switch (axis)
	{
		case POCKET3_AXIS_LEFT_ANALOG_HORIZONTAL:
		...
		case POCKET3_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case POCKET3_AXIS_RIGHT_ANALOG_VERTICAL:
			return POCKET3_ANALOG_VALUE_MAX;
		default:
			break;
	}
...
static int translate_scancode(int scancode)
{
	switch (scancode)
	{
		case POCKET3_SCANCODE_HOME:
			return MERGED_SCANCODE_HOME;
		case POCKET3_SCANCODE_A:
			return MERGED_SCANCODE_B;
		case POCKET3_SCANCODE_B:
			return MERGED_SCANCODE_A;
		case POCKET3_SCANCODE_Y:
			return MERGED_SCANCODE_X;
		case POCKET3_SCANCODE_X:
			return MERGED_SCANCODE_Y;
		case POCKET3_SCANCODE_L1:
			return MERGED_SCANCODE_L1;
		case POCKET3_SCANCODE_R1:
			return MERGED_SCANCODE_R1;
		case POCKET3_SCANCODE_SELECT:
			return MERGED_SCANCODE_SELECT;
		case POCKET3_SCANCODE_START:
			return MERGED_SCANCODE_START;
		case POCKET3_SCANCODE_L3:
			return MERGED_SCANCODE_L3;
		case POCKET3_SCANCODE_R3:
			return MERGED_SCANCODE_R3;
		case POCKET3_SCANCODE_VOLUME_MINUS:
			return MERGED_SCANCODE_VOLUME_MINUS;
		case POCKET3_SCANCODE_VOLUME_PLUS:
			return MERGED_SCANCODE_VOLUME_PLUS;
		case POCKET3_SCANCODE_POWER:
...
static void scale_and_translate_axis(struct input_event * ev)
{
	...
	// Scale range
	int32_t new_value = scale_axis_value(ev->value, get_pocket3_axis_min_value(ev->code), get_pocket3_axis_max_value(ev->code), get_merged_axis_min_value(new_code), get_merged_axis_max_value(new_code));
...
void *simulate_volumekey_repeat_pocket3(void *volumekey_struct)
{
...
void merge_pocket3_inputs(int merged_gamepad)
{
	...
	int gpio_keys = open(POCKET3_GPIO_KEYS, O_RDONLY | O_NONBLOCK);
	...
	int gamepad = open(POCKET3_GAMEPAD, 0x102);
			...
			if (FD_ISSET(gpio_keys, &wfds))
			{
				...
				if (read(gpio_keys, &ev, sizeof(ev)) == sizeof(ev))
				{
					...
					if (ev.code == KEY_VOLUMEUP)
					{
						...
						{
							pthread_create(&volumeup_thread, NULL, &simulate_volumekey_repeat_pocket3, (void *)&volumeup_struct);
						}
					}
					if (ev.code == KEY_VOLUMEDOWN)
					{
						...
						{
							pthread_create(&volumedown_thread, NULL, &simulate_volumekey_repeat_pocket3, (void *)&volumedown_struct);
						}
					}
