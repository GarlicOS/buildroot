...
static int32_t get_rg405_axis_min_value(int axis)
{
	switch (axis)
	{
		case RG405_AXIS_LEFT_ANALOG_HORIZONTAL:
		case RG405_AXIS_LEFT_ANALOG_VERTICAL:
		case RG405_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case RG405_AXIS_RIGHT_ANALOG_VERTICAL:
			return RG405_ANALOG_VALUE_MIN;
		case RG405_AXIS_DPAD_HORIZONTAL:
		case RG405_AXIS_DPAD_VERTICAL:
			return RG405_DPAD_VALUE_MIN;
		default:
			break;
	}
...
static int32_t get_rg405_axis_max_value(int axis)
{
	switch (axis)
	{
		case RG405_AXIS_LEFT_ANALOG_HORIZONTAL:
		...
		case RG405_AXIS_RIGHT_ANALOG_HORIZONTAL:
		case RG405_AXIS_RIGHT_ANALOG_VERTICAL:
			return RG405_ANALOG_VALUE_MAX;
		case RG405_AXIS_DPAD_HORIZONTAL:
		case RG405_AXIS_DPAD_VERTICAL:
			return RG405_DPAD_VALUE_MAX;
		default:
			break;
	}
...
static void scale_and_translate_axis(struct input_event * ev)
{
	...
	// Scale range
	int32_t new_value = scale_axis_value(ev->value, get_rg405_axis_min_value(ev->code), get_rg405_axis_max_value(ev->code), get_merged_axis_min_value(new_code), get_merged_axis_max_value(new_code));
...
static int convert_to_axis_event(struct input_event * ev)
{
	switch (ev->code)
	{
		case RG405_SCANCODE_L2:
		case RG405_SCANCODE_R2:
			// Convert digital to analog triggers
			ev->type = EV_ABS;
			ev->code = ev->code == RG405_SCANCODE_L2 ? MERGED_AXIS_LEFT_TRIGGER : MERGED_AXIS_RIGHT_TRIGGER;
...
static void input_event(int fd, struct input_event * ev)
{
	...
	// Filter out repeat events (these occur on the RG405_GPIO_KEYS input device)
...
void merge_rg405_inputs(int merged_gamepad)
{
	...
	int gpio_keys = open(RG405_GPIO_KEYS, O_RDONLY | O_NONBLOCK);

	// Open the retrogame_joypad (houses everything else)
	int gamepad = open(RG405_GAMEPAD, O_RDONLY | O_NONBLOCK);
